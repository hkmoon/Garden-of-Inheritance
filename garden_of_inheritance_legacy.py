...
import icon_loader
from garden_of_inheritance.bootstrap import (
    configure_logging,
    get_base_dir,
    grid_config_path,
    load_grid_config,
...
_PG_BASE_DIR = get_base_dir(__file__)
_PG_LOG_PATH = configure_logging(_PG_BASE_DIR)
...
__PG_BASE_DIR = get_base_dir(__file__)
...
def resource_path(*parts) -> Path:
    ...
    """
    return resolve_resource_path(__PG_BASE_DIR, *parts)
...
SOIL_COLORS = list(theme.SOIL_COLORS)
...
SEEDS_CSV = os.path.join(icon_loader.ICONS_DIR, "seeds.csv")  # keep next to icons for convenience
...
# ============================================================================
    ...
    def _ensure_tile_icon(self, plant, selected=False):
        ...
        try:
            ...
            icon_path = None
            try:
                icon_path = icon_loader.stage_icon_path_for_plant(plant)
            except Exception:
                icon_path = None

            # fallback
            if not icon_path or not isinstance(icon_path, str) or not os.path.exists(icon_path):
                fb = os.path.join(icon_loader.ICONS_DIR, "plant.png")
            ...
            img = icon_loader.safe_image(icon_path)
            if img is None:
                return
...
# ============================================================================
    def __init__(self, root: tk.Tk):
        ...
        theme.apply_window_options(self.root)
        ...
        self.field_bg = theme.FIELD_BG
        self.grid_bg = self.panel_bg
        self._build_ui()
    ...
    def _choose_grid_bg(self):
        ...
        try:
            color = colorchooser.askcolor(
                ...
                initialcolor=getattr(self, "field_bg", self.grid_bg)
            )[1]
        except Exception:
            color = None
        ...
        try:
            for j, cell in enumerate(self.tiles):
                soil = getattr(self, "tile_soil_colors", {}).get(j, color)
                cell.configure(
                    bg=color,
                    highlightbackground=theme.FIELD_EDGE,
                    highlightcolor=theme.FIELD_EDGE,
                )
        except Exception:
            pass

        for j, tile in enumerate(self.tiles):
            tile.soil = getattr(self, "tile_soil_colors", {}).get(j, tile.soil)
...
# ============================================================================
    ...
    def _build_ui(self):
        ...
        menu_font = theme.font_spec(theme.UI_FONT, 11)
        self.menubar = tk.Menu(self.root, font=menu_font) if not hasattr(self, "menubar") else self.menubar
        ...
        self.load_submenu = tk.Menu(file_menu, tearoff=0, font=menu_font, postcommand=lambda: self._build_load_menu(self.load_submenu))
        ...
        game_menu = tk.Menu(self.menubar, tearoff=0, font=menu_font)
        ...
        wildlife_menu = tk.Menu(game_menu, tearoff=0, font=menu_font)
        ...
        difficulty_menu = tk.Menu(game_menu, tearoff=0, font=menu_font)
        ...
        self.topbar = tk.Frame(self.root, padx=8, pady=6, bg=theme.APP_BG)
        self.topbar.pack(fill="x")

        # Font setup
        self.font_seed   = (theme.DISPLAY_FONT, 15, "bold")
        self.font_law    = (theme.UI_FONT, 13, "bold")

        # Modern UI font + shared button style
        self.font_button = (theme.UI_FONT, 11, "bold")
        self.button_fg = theme.BUTTON_TEXT_FG
        self.button_style = {
        ...
        def _hover_on(event):
            ...
            if hover:
                ...
                w.configure(bg=getattr(w, "_hover_bg", theme.BUTTON_HOVER))
        ...
        def _hover_off(event):
            ...
            if base:
                ...
                w.configure(bg=theme.BUTTON_BG)
        ...
        self.content = tk.Frame(self.root, padx=8, pady=8, bg=theme.APP_BG)
        ...
        mendel_img = icon_loader.safe_image(os.path.join(icon_loader.ICONS_DIR, "mendel.png"))
        self.mendel_label = tk.Label(self.left_panel, image=mendel_img, bg=self.panel_bg)
        ...
        self.icon_row = tk.Frame(self.left_panel, bg=self.panel_bg)
        ...
        self.plant_icon_label = tk.Label(self.icon_row, bg=self.panel_bg)
        ...
        # Separator line
        tk.Frame(self.left_panel, height=2, bg=theme.PANEL_BORDER).pack(fill="x", pady=(2, 2))

        # Centered "Traits" header
        self.traits_header = tk.Label(
            self.left_panel,
            text="Traits:",
            font=(theme.DISPLAY_FONT, 13, "bold"),
            bg=self.panel_bg,
            fg=theme.TEXT_PRIMARY,
        ...
        self.traits_container = tk.Frame(self.left_panel, bg=self.panel_bg)
        ...
        self.status_msg = tk.Label(
            ...
            highlightbackground=theme.STATUS_BORDER,
            padx=6,
            pady=4,
        )
        ...
        self.inventory_row = tk.Frame(right_panel, bg=self.panel_bg)
        ...
        self.grid_frame = tk.Frame(right_panel, padx=8, pady=8, bg=self.field_bg)
        ...
        law_left = tk.Frame(self.law_row, bg=self.panel_bg)
        ...
        try:
            ...
            if hasattr(self, "law_status_label"):
                if not (law1_done or law2_done or law3_done):
                    ...
                    self.law_status_label.configure(fg=theme.TEXT_MUTED)
                else:
                ...
                if k in plant.revealed_traits:
                    ...
                    if k == "flower_position":
                        try:
                            ...
                            if pos and col:
                                hi = icon_loader.flower_icon_path_hi(pos, col)
                                if hi:
                                    img = icon_loader.safe_image(hi)
                    ...
                    if not icon_used and k == "pod_shape":
                        try:
                            ...
                            if shape and col:
                                p = icon_loader.pod_shape_icon_path(shape, col)
                                if p:
                                    img = icon_loader.safe_image(p)
                                    icon_lbl = tk.Label(cell, image=img)
                                    icon_lbl.image = img
                                    icon_lbl.pack(anchor="center")
                                    icon_used = "pod"
                            ...
                            pass
                    if not icon_used:
                        icon_path = icon_loader.trait_icon_path(k, v)
                        if icon_path:
                            img = icon_loader.safe_image(icon_path)
                            icon_lbl = tk.Label(cell, image=img)
                            icon_lbl.image = img
...
# ============================================================================
    def _on_genetics(self):
        ...
        def _table_row(tbl, r, trait_label, icon_path, chrom_text, allele_text, why):
            ...
            img = icon_loader.safe_image_scaled(icon_path, 2, 2)
        ...
        def _fill_tab(frame, plant_obj, label_hint):
            ...
            icon_fc = icon_loader.trait_icon_path("flower_color", fc)
            ...
            icon_phgt = icon_loader.trait_icon_path("plant_height", phgt)
            ...
            icon_pod = icon_loader.pod_shape_icon_path(pod_shape, pod_color)
            ...
            icon_sd_shape = icon_loader.trait_icon_path("seed_shape", sd_shape)
            ...
            icon_sd_color = icon_loader.trait_icon_path("seed_color", sd_color)
            ...
            fc = ph.get("flower_color", "")
            fp_icon = icon_loader.flower_icon_path(fp, fc) or icon_loader.trait_icon_path("flower_position", fp)
    ...
    def choose_seed_for_tiles(self, tiles: List[TileCanvas]):
        ...
        def _render():
            ...
            for i, (kind, src, donor, count, label, match_fn) in enumerate(shown):
                ...
                btn_group_x._hover_bg = theme.BUTTON_DANGER_HOVER
                ...
                if seed_color:
                    try:
                        ...
                        icon_path = os.path.join(icon_loader.ICONS_DIR, icon_filename)
                        ...
                        if os.path.exists(icon_path):
                            ...
                            plant_seeds_path = os.path.join(icon_loader.ICONS_DIR, "plant-seeds.png")
                            if os.path.exists(plant_seeds_path):
                                img2 = icon_loader.safe_image(plant_seeds_path)
                ...
                b_plant_n = tk.Button(
                    ...
                    **{
                        **self.button_style,
                        "bg": theme.BUTTON_SUCCESS_BG,
                        "activebackground": theme.BUTTON_SUCCESS_HOVER,
                        "fg": self.button_fg,
                        "activeforeground": self.button_fg,
                    },
                )
                self._apply_hover(b_plant_n)
                b_plant_n._hover_bg = theme.BUTTON_SUCCESS_HOVER
                ...
                b_all._hover_bg = theme.BUTTON_SUCCESS_HOVER
                ...
                b_del._hover_bg = theme.BUTTON_DANGER_HOVER
...
# ============================================================================
    ...
    def _build_load_menu(self, parent_menu):
        ...
        save_listing = list_save_files(data_dir)
        named_saves = save_listing["named"]
        unnamed_saves = save_listing["unnamed"]
        ...
        if named_saves:
            ...
            for save_info in named_saves:
                parent_menu.add_command(
                    label=save_info["display_name"],
                    command=lambda fp=save_info["filepath"]: self._load_garden_from_file(fp)
                )
        ...
        if unnamed_saves:
            ...
            for save_info in unnamed_saves:
                parent_menu.add_command(
                    label=save_info["display_name"],
                    command=lambda fp=save_info["filepath"]: self._load_garden_from_file(fp)
                )
    ...
    def _serialize_garden_state(self):
        ...
        return serialize_garden_state(self, ROWS, COLS)
    
    def _deserialize_garden_state(self, save_data):
        """Restore garden state from serialized data."""
        deserialize_garden_state(self, save_data, ROWS, COLS)
...
# ============================================================================
    ...
    def archive_snapshot(self, plant, **extras):
        ...
        if not hasattr(self, "archive"):
            self.archive = {}
        return upsert_archive_snapshot(self.archive, plant, extras)



def _grid_config_path():
    return grid_config_path(__file__)


def _load_grid_config():
    return load_grid_config(__file__, ROWS, COLS)


def _save_grid_config(rows, cols, show_dialog):
    save_grid_config(__file__, rows, cols, show_dialog)
